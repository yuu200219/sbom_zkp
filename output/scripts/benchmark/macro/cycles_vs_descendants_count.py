...
def main():
    ...
    ax1.bar(df_agg['descendants_count'], df_agg['user_cycles'], color='indigo', alpha=0.8, label='Avg User Cycles')
    ax1.bar(df_agg['descendants_count'], df_agg['overhead_cycles'], bottom=df_agg['user_cycles'], color='deeppink', alpha=0.8, label='Avg Overhead Cycles')
    
    ax1.set_title("Cycles vs Descendants Count")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'cycles_vs_descendants_count.png'), dpi=300, bbox_inches='tight')
    print("Saved cycles_vs_descendants_count.png")
