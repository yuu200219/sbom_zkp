...
def main():
    ...
    df = df.dropna(subset=['children_count', 'verify_duration'])
    ...
    df_agg.columns = ['children_count', 'mean_verify_duration', 'std_verify_duration', 'mean_receipt_size']
    
    fig, ax1 = plt.subplots(figsize=(12, 7))
    sns.scatterplot(data=df, x='children_count', y='verify_duration', hue='source', palette='viridis', alpha=0.3, ax=ax1, legend=False)
    
    # Plot Mean and Std
    ax1.errorbar(df_agg['children_count'], df_agg['mean_verify_duration'], yerr=df_agg['std_verify_duration'], 
    ...
    ax1.set_title("Verification Duration vs Children Count")
    ...
    ax2 = ax1.twinx()
    sns.lineplot(data=df_agg, x='children_count', y='mean_receipt_size', marker='^', markersize=8, color='crimson', linewidth=2.0, linestyle='--', ax=ax2, label='Avg Receipt Size')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'verify_duration_vs_children_count.png'), dpi=300, bbox_inches='tight')
    print("Saved verify_duration_vs_children_count.png")
