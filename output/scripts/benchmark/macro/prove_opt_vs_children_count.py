...
def main():
    ...
    sns.scatterplot(data=df, x='children_count', y='total_prove_duration', hue='source', palette='viridis', alpha=0.7, ax=ax1)
    sns.lineplot(data=df_agg, x='children_count', y='total_prove_duration', color='darkcyan', marker='o', linewidth=2.5, label='Avg Total Proving Duration', ax=ax1)
    
    ax1.set_title("End-to-End Proving Duration vs. Children Count")
    ax1.set_xlabel("Children Count")
    ax1.set_ylabel("Total Proving Duration (ms) \n[Base STARK + Compression]", color='darkcyan')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'prove_duration_opt_vs_children_count.png'), dpi=300, bbox_inches='tight')
    print("Saved prove_duration_opt_vs_children_count.png")
