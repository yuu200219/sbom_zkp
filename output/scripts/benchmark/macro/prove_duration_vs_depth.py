...
def main():
    ...
    sns.scatterplot(data=df, x='depth', y='pure_prove_duration', hue='source', palette='viridis', alpha=0.7, ax=ax1)
    sns.lineplot(data=df_agg, x='depth', y='pure_prove_duration', color='darkcyan', marker='o', linewidth=2.5, label='Avg Proving Duration', ax=ax1)
    
    ax1.set_title("Proving Duration vs Depth")
    ax1.set_xlabel("Depth")
    ax1.set_ylabel("Proving Duration (ms)", color='darkcyan')
    ax1.tick_params(axis='y', labelcolor='darkcyan')
    
    ax2 = ax1.twinx()
    sns.lineplot(data=df_agg, x='depth', y='receipt_size_kb', marker='^', markersize=8, color='crimson', linewidth=2.0, linestyle='--', ax=ax2, label='Avg Receipt Size')
    sns.scatterplot(data=df, x='depth', y='receipt_size_kb', color="crimson", alpha=0.3, ax=ax2, legend=False)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'prove_duration_vs_depth.png'), dpi=300, bbox_inches='tight')
    print("Saved prove_duration_vs_depth.png")
