...
def main():
    ...
    sns.lineplot(
        data=df_prove_agg, x='descendants_count', y='total_prove_duration', 
        marker='o', markersize=8, color='#3498db', linewidth=2.5, linestyle=':', 
        ax=ax, label='Avg Proving Duration (prover.prove)'
    )
    sns.scatterplot(
        data=df_prove, x='descendants_count', y='total_prove_duration', 
        color='#3498db', alpha=0.3, ax=ax, legend=False
    )
    
    # 2. 繪製 prover_with_opts 的數據
    sns.lineplot(
        data=df_opt_agg, x='descendants_count', y='total_prove_duration', 
        marker='s', markersize=8, color='#e74c3c', linewidth=2.5, linestyle='--', 
        ax=ax, label='Avg Proving Duration (prover_with_opts)'
    )
    sns.scatterplot(
        data=df_opt, x='descendants_count', y='total_prove_duration', 
        color='#e74c3c', alpha=0.3, ax=ax, legend=False
    )
    
    ax.set_title("Proving Duration Comparison: prover.prove vs. prover_with_opts", pad=15, fontweight='bold')
    ax.set_xlabel("Descendants Count")
    ax.set_ylabel("Total Proving Duration (min)")
    ax.grid(True, linestyle='--', alpha=0.6)
    
    plt.legend(loc='upper left')
    plt.tight_layout()
    
    output_path = os.path.join(OUTPUT_DIR, 'prove_duration_vs_descendants_count_prove_vs_opt.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Saved proving duration comparison graph to: {output_path}")
