...
def main():
    ...
    sns.scatterplot(data=df, x='{x_axis}', y='pure_prove_duration', hue='source', palette='viridis', alpha=0.7, ax=ax1)
    sns.lineplot(data=df_agg, x='{x_axis}', y='pure_prove_duration', color='darkcyan', marker='o', linewidth=2.5, label='Avg Proving Duration', ax=ax1)
    
    ax1.set_title("Proving Duration vs {title_x}")
    ax1.set_xlabel("{title_x}")
    ax1.set_ylabel("Proving Duration (ms)", color='darkcyan')
    ax1.tick_params(axis='y', labelcolor='darkcyan')
    
    ax2 = ax1.twinx()
    sns.lineplot(data=df_agg, x='{x_axis}', y='receipt_size_kb', marker='^', markersize=8, color='crimson', linewidth=2.0, linestyle='--', ax=ax2, label='Avg Receipt Size')
    sns.scatterplot(data=df, x='{x_axis}', y='receipt_size_kb', color="crimson", alpha=0.3, ax=ax2, legend=False)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'prove_duration_vs_{x_axis}.png'), dpi=300, bbox_inches='tight')
...
def main():
    ...
    fig, ax1 = plt.subplots(figsize=(12, 7))
    sns.scatterplot(data=df, x='{x_axis}', y='total_cycles', hue='source', palette='viridis', alpha=0.3, ax=ax1, legend=False)
    
    sns.lineplot(data=df_agg, x='{x_axis}', y='total_cycles', color='darkviolet', marker='o', linewidth=2.5, label='Avg Total Cycles', ax=ax1)
    sns.lineplot(data=df_agg, x='{x_axis}', y='user_cycles', color='indigo', marker='s', linewidth=2.5, label='Avg User Cycles', ax=ax1)
    sns.lineplot(data=df_agg, x='{x_axis}', y='overhead_cycles', color='deeppink', marker='d', linewidth=2.5, label='Avg Overhead Cycles', ax=ax1)
    
    ax1.set_title("Cycles vs {title_x}")
    ax1.set_xlabel("{title_x}")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'cycles_vs_{x_axis}.png'), dpi=300, bbox_inches='tight')
...
for x_axis, title_x in [("children_count", "Children Count"), ("depth", "Depth"), ("descendants_count", "Descendants Count")]:
    ...
    with open(os.path.join(MACRO_DIR, f"prove_duration_vs_{x_axis}.py"), "w") as f:
        f.write(COMMON_HEADER + PROVE_DURATION_TEMPLATE.replace("{x_axis}", x_axis).replace("{title_x}", title_x).replace("{filter_code}", filter_code))
    with open(os.path.join(MACRO_DIR, f"cycles_vs_{x_axis}.py"), "w") as f:
        f.write(COMMON_HEADER + CYCLES_TEMPLATE.replace("{x_axis}", x_axis).replace("{title_x}", title_x).replace("{filter_code}", filter_code))
