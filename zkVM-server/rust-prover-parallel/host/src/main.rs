...
async fn run_risc0_prover(
    ...
    let (receipt, cycles, pure_prove_duration) = tokio::task::spawn_blocking(move || -> Result<(Receipt, u64, f64), String> {
        ...
        if comp_input.comp_type == "virtual-batch" {
            ...
            metrics::gauge!("last_compression_duration_ms", &labels_for_task).set(compress_duration);
            
            Ok((compressed_receipt, cycles, pure_prove_duration))
        } else {
            Ok((prove_info.receipt, cycles, pure_prove_duration))
        }
    })
    .await
    ...
    metrics::gauge!("last_proving_duration_ms", &labels).set(pure_prove_duration);
    ...
    metrics::gauge!("last_proving_cycles", &labels).set(cycles as f64);
