...
app.post('/api/generate-and-prove', upload.single('file'), async (req, res) => {
    // 取得 proof 請求資料
    const file = req.file;
    const { artifactId } = req.body;
    if (!file)
        return res.status(400).json({ error: '請上傳 lockfile (如 package-lock.json)' });
    try {
        console.log(`[Node] 1. 正在轉發檔案至 SBOM Service: ${file.originalname}`);
        // --- 1. 轉發檔案給 sbom-service ---
        const form = new FormData();
        form.append('file', fs.createReadStream(file.path), file.originalname);
        const sbomRes = await axios.post(`${SBOM_SERVICE_URL}/generate`, form, {
            headers: { ...form.getHeaders() }
        });
        const { merkleRoot, sbomGenerationMs } = sbomRes.data;
        ...
        res.status(200).json({
            ...
            merkleRoot,
            time: {
                'sbomGenerationMs': sbomGenerationMs,
                'proveDurationMs': response.data.proveDurationMs,
                'ipfsUploadMs': ipfsTime,
                'totalProcessTimeMs': sbomGenerationMs + response.data.proveDurationMs + ipfsTime
            },
            ipfs: ipfsHash ? {
                cid: ipfsHash,
...
app.post('/api/prove', async (req, res) => {
    ...
    try {
        ...
        if (!fs.existsSync(storagePath))
            fs.mkdirSync(storagePath); // 確保資料夾存在
        const fileName = `proof-${artifactId}-${Date.now()}.json`;
        const fileContent = JSON.stringify({ artifactId, proof, journal }, null, 2);
        fs.writeFileSync(path.join(storagePath, fileName), fileContent);
        console.log(`💾 Proof 已存檔至: proofs/${fileName}`);
